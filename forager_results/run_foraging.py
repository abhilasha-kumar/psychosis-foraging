...
switch_methods = ['simdrop','multimodal','norms_associative', 'norms_categorical', 'delta','all', 'multimodaldelta',
                  'troyerextended']
...
def retrieve_data(path):
    ...
    data = prepareDataWithCorrections(path)
    return data

def get_lexical_data():
    
    troyerexpath = 'data/norms/troyer_extended_lundin_2024_03_22.csv'
    ...
    troyerextendednorms = pd.read_csv(troyerexpath, encoding="unicode-escape")
    ...
    return norms, troyerextendednorms, similarity_matrix, phon_matrix, frequency_list,labels
...
def calculate_switch(switch, fluency_list, semantic_similarity, phon_similarity, norms, troyerextendednorms, alpha = np.arange(0, 1.1, 0.1), rise = np.arange(0, 1.25, 0.25), fall = np.arange(0, 1.25, 0.25)):
    '''
    1. Check if specified switch model is valid
    2. Return set of switches, including parameter value, if required

    switch_methods = ['simdrop','multimodal','norms_associative', 'norms_categorical', 'delta','all', 'multimodaldelta', 'troyerextended']
    '''
    switch_names = []
    switch_vecs = []
    ...
    if switch == switch_methods[6] or switch == switch_methods[5]:
        for i, a in enumerate(alpha):
            for i, r in enumerate(rise):
                for j, f in enumerate(fall):
    ...
    if switch == switch_methods[7] or switch == switch_methods[5]:
        switch_names.append(switch_methods[7])
        switch_vecs.append(switch_norms_troyerextended(fluency_list,troyerextendednorms))
            

    return switch_names, switch_vecs

def run_model(data, model_type, history_vars_list, switch_names_list, switch_vecs_list):
    # Get Lexical Data needed for executing methods
    norms, troyerextendednorms, similarity_matrix, phon_matrix, frequency_list, labels = get_lexical_data()
    forager_results = []
    ...
    for i, (subj, fl_list) in enumerate(tqdm(data)):
        ...
        #switch_names, switch_vecs = calculate_switch(switch_type, fl_list, history_vars[0],   history_vars[4], norms, troyerextendednorms)
...
def run_lexical(data, corrected_df):
    # Get Lexical Data needed for executing methods
    norms, troyerextendednorms, similarity_matrix, phon_matrix, frequency_list, labels = get_lexical_data()
    lexical_results = []
    history_vars_list = []
...
def run_switches(data,switch_type, history_vars_list):
    norms, troyerextendednorms, similarity_matrix, phon_matrix, frequency_list, labels = get_lexical_data()
    switch_results = []
    switch_names_list = []
    switch_vecs_list = []
...
if args.pipeline == 'evaluate_data':
    data, evaluation_df, processed_df, corrected_df = retrieve_data(args.data)
    ...
    lexical_results, history_vars_list = run_lexical(data, corrected_df)
    ...
    switch_results, switch_names_list, switch_vecs_list = run_switches(data,args.switch, history_vars_list)
    ...
    # Run subroutine for getting model outputs
    print("Checking Data ...")
    data, replacement_df, processed_df, corrected_df = retrieve_data(args.data)
    ...
    model_results = run_model(data, args.model, history_vars_list, switch_names_list, switch_vecs_list)
...
#### SAMPLE RUN CODE ####
# python run_foraging.py --data data/fluency_lists/nancy_V1_raw.txt --pipeline models --switch all --model all