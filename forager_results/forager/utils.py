...
def prepareDataWithCorrections(path):
    ...
    # load vocab labels
    labels = pd.read_csv('data/lexical_data/vocab.csv', names=['word'], header=None) 

    # make corrections based on corrections file

    processed_df = df.copy()

    corrections = pd.read_excel('data/fluency_lists/final_corrections.xlsx')
    ...
    vocab = labels['word'].values
    # exclude mammal from vocab
    vocab = [w for w in vocab if w != "mammal"]
    oov = [w for w in values if w not in vocab]
    #print(oov)
    if len(oov) > 0:
        ...
        for word in oov:
            ...
            # first check corrections file for whether to exclude or replace
            ...
            if correction.shape[0] > 0:
                if correction['final_evaluation'].values[0] == 'EXCLUDE':
                    exclude(word, evaluation_df)
                    replacements[word] = "EXCLUDE"
                elif correction['final_evaluation'].values[0] == 'REPLACE':
                    replacements[word] = correction['final_word'].values[0]
            else:
        ...
        # also exclude any rows where the entry is "EXCLUDE"
        processed_df = processed_df[processed_df['entry'] != "EXCLUDE"]
        ...
        # count how many "exclude" are in evaluation_df
        ...
        # drop "EXCLUDE" rows from corrected_df (only need replacements for create_history)
        corrected_df = corrected_df[corrected_df['evaluation'] != "EXCLUDE"]
