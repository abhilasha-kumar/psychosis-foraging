...
def switch_norms_associative(fluency_list,norms):
    ...
    for k in range(len(fluency_list)):
        if k > 0:
            ...
            #print(f"category for {item1}: {category1}")
            #print(f"category for {item2}: {category2}")
...
def switch_norms_troyerextended(fluency_list,norms):
    '''
        Switch Method Based on EXTENDED Troyer Norms from Lundin et al. 2023.
